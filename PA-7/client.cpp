...
int main(int argc, char * argv[]) {
    ...
    string ipc = "n"; // default IPC is network
	string host = "local"; // default is local host
	string port = "22"; // default port is 22
    int opt = 0;
    while ((opt = getopt(argc, argv, "n:w:b:i:p:h:")) != -1) {
        switch (opt) {
            case 'n':
                n = atoi(optarg);
                break;
	...
	if (ipc == "n") chan = (RequestChannel*) new NetworkRequestChannel("control",RequestChannel::CLIENT_SIDE,host,port);
	...
	for (int i = 0; i < w; i++) {
		...
		if (ipc == "n") workerChannel = (RequestChannel*) new NetworkRequestChannel(s, RequestChannel::CLIENT_SIDE, host, port);
